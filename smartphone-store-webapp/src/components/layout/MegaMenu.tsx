...
const brandConfig = [
  { key: 'apple', labelKey: 'common.brandApple', color: 'from-gray-400 to-gray-600' },
  { key: 'samsung', labelKey: 'common.brandSamsung', color: 'from-blue-400 to-blue-600' },
  { key: 'xiaomi', labelKey: 'common.brandXiaomi', color: 'from-orange-400 to-orange-600' },
] as const
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Invisible bridge to prevent gap between nav button and menu */}
          ...
          <div className="mx-auto w-full max-w-7xl rounded-2xl glass shadow-2xl border border-glass-border overflow-hidden bg-popover/95 backdrop-blur-xl">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0">
              {brandConfig.map((brand) => {
                const products = getProductsByBrand(brand.key).slice(0, 3)
                return (
                  <div
                    key={brand.key}
                  ...
                  >
                    ...
                    >
                      <div className={cn('w-2 h-2 rounded-full bg-gradient-to-r shrink-0', brand.color)} />
            ...
            <div className="border-t border-border px-5 lg:px-6 py-4 flex justify-between items-center bg-muted/30">
              <p className="text-sm text-muted-foreground">{t('nav.brands')}</p>
              <Link
                to="/products"
                onClick={onClose}
